...
a = 8000*1000           # [m] chief semi-major axis
e = 0.1                 # chief eccentricity
i = 80*pi/180           # [rad] chief inclination

# Deputy relative orbit elements
delta_a = 10*1000       # [m]
delta_e = 0.05
delta_i = 1*pi/180

# Mean differential ascending node change dela_ohm over one orbitp period
eps = 3*J2*(r_eq/(a*(1-e**2)))**2
n = sqrt(mu/a**3)       # Chief mean orbit rate in rad/s
period = 2*pi/n         # Chief orbit period (seconds)
eta = sqrt(1 - e**2)
delta_kappa_ohm = 7/4*cos(i)*delta_a/a - 2*(e/eta**2)*cos(i)*delta_e + 0.5*sin(i)*delta_i
delta_ohm_rate = eps*n*delta_kappa_ohm
delta_ohm_change_orbit = delta_ohm_rate*period

print("delta_ohm change over one orbit = ", delta_ohm_change_orbit*180/pi, "degrees")